...
static void generate_events(struct tcase *tc)
{
	...
	if (tc->mask & FAN_ONDIR) {
		DIR *dir = SAFE_OPENDIR(MOUNT_PATH);
		...
		TST_EXP_FAIL(faccessat(dirfd(dir), notfound, F_OK, 0),
			     ENOENT, "faccessat(%d, notfound, F_OK, 0)", dirfd(dir));
		event++;
	...
	while (test_num < EVENT_SET_MAX && fd_notify != -1) {
		...
		if (event->fd != FAN_NOFD) {
			...
			if (!(tc->mask & FAN_ONDIR)) {
				 ...
				 */
				sprintf(notfound, MOUNT_PATH"/nonotify_%d", (int)(tc - tcases));
				TST_EXP_FAIL(faccessat(event->fd, notfound, F_OK, 0),
					     ENOENT, "faccessat(%d, nonotify, F_OK, 0)", event->fd);
			}
			SAFE_CLOSE(event->fd);
		}
