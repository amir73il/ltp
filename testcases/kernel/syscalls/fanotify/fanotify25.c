...
static int setup_mark(unsigned int n)
{
	...
	fd_notify = SAFE_FANOTIFY_INIT(FAN_CLASS_PRE_CONTENT |
				       FAN_REPORT_DFID_NAME_TARGET, O_RDONLY);
...
static void test_fanotify(unsigned int n)
{
	...
	while (test_num < EVENT_SET_MAX && fd_notify != -1) {
		...
		struct file_handle *file_handle, *child_fh = NULL;
		unsigned int fhlen;
		const char *filename;
		int namelen;
		...
		if (((char *)child_fid - (char *)event) >= event->event_len)
			...
			child_fh = (struct file_handle *)child_fid->handle;
		...
		if (event->mask != event_set[test_num].mask) {
			...
			tst_res(TFAIL,
				...
				event->fd);
		} else if (event_fid->hdr.info_type != FAN_EVENT_INFO_TYPE_DFID_NAME ||
			   !fhlen || namelen < 0) {
			tst_res(TFAIL,
		...
		if (child_fh) {
			...
			event->fd = open_by_handle_at(bind_mount_fd, child_fh, O_RDONLY);
			if (event->fd < 0)
